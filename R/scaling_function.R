...
#' @param cats = should the function convert categorical variables into dummies?
#' @keywords multimodel, demean, scale
#' @export
#' @examples
#' scaler_cat

scaler_cat<-function(df, ID, centered=TRUE, scaled=TRUE, cats = TRUE, ...){
...
#--------------------bind numeric and categorical together with ID.vars-----------------#
	scaled.df<-cbind(ID.vars, data.frame(scaled_cont), scaled_cat[,-1])
	return(scaled.df)
}
