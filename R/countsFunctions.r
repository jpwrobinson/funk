...
binData = function(x = NULL, counts = NULL, binWidth = NULL, binBreaks = NULL,
                   startInteger = TRUE, integer=TRUE)
    {
    ...
    #  INT: are counts integers (TRUE) or non-integers (FALSE)?
    # Returns:
    #  list containing:
    #   
...
## Bin details for non-integer count data
        
        indiv<-counts

        indiv$binMin = counts[,'mass']
        indiv$binMid = counts[,'mass']+binWidth/2
        indiv$binMax = counts[,'mass']+binWidth
...
calcLike = function(negLL.fn, p, vecDiff=0.5, vecInc=0.001, ...)
    ...
    minLL = nlm(negLL.fn, p=p, ...)
        #x=binMids,
        #c=binCounts, K=num.bins, xmin=xmin, xmax=xmax,
        #sumclogx=sumCntLogMids)
   
    MLE = minLL$estimate

    conf = profLike(negLL.fn, MLE, minLL$minimum, ...)
        #x=binMids, c=binCounts, K=num.bins,
        #             xmin=xmin, xmax=xmax, sumclogx=sumCntLogMids)
